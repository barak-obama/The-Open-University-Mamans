...
public class Circle {
	public static void main(String[] args) {
		...
		System.out.printf("Incircle: radius = %s, area = %s, perimeter = %s\n", innerRadius, innerArea,innerPerimeter);
		
		int width = rightBottomX - leftUpX;
		double externalRadius = Math.sqrt(Math.pow(width, 2) + Math.pow(height, 2)) / 2;
		double externalArea = Math.PI * Math.pow(externalRadius, 2),
				externalPerimeter = 2 * Math.PI * externalRadius;
		
		System.out.printf("Excircle: radius = %s, area = %s, perimeter = %s", externalRadius, externalArea,externalPerimeter);
